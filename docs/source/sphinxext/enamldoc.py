...
class EnamlComponentDocumenter(ModuleLevelDocumenter):
    ...
    def __init__(self, directive, name, indent=u''):
        """
        Need to override the parent __init__ so that we can set self.domain,
...
class EnamlDeclarationDocumenter(EnamlDocstringSignatureMixin,
    ...
    def format_args(self):
        """
        Derivation is shown where normally arguments are shown in a function
        prototype.
        """
        return '({0})'.format(self.object.__base__)
...
class EnamlDefnDocumenter(EnamlDocstringSignatureMixin,
    ...
    def document_members(self, all_members=False):
        """
        We may want to use this in the future to include submembers of a defn.
        """
        pass
