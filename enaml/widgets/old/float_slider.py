...
class FloatSlider(Slider):
    ...
    #: the current position is set to :attr:minimum. Default is 0.0.
    ...
    #: then the current position is set to :attr:maximum. Default is 1.0.
    maximum = Property(Float, depends_on ='_maximum')

    #: The internal maximum storage.
    _maximum = Float(1.0)

    #: The number of integer steps allowed by the slider. Default is 100.
    precision = Range(value=100, low=1, high=MAX_SLIDER_VALUE)

    #: The interval steps to put between tick marks. Default is 10.
    tick_interval = Range(
        low='_one', high='precision', value=10, exclude_high=True,
    )
    ...
    #: An internal attribute to make the traits Range object happy when 
    #: looking up the bounds of a range.
    _one = Constant(1)
    ...
    def _get_minimum(self):
        ...
        Even though this is identical to the parent class method, it is 
        required since we redefine the property on this class, and 
        traits doesn't like that too much.
    ...
    def _set_minimum(self, value):
        ...
        :attr:`maximum`. This is an overridden parent class method.
    ...
    def _set_maximum(self, value):
        ...
        :attr:`minimum`. This is an overridden parent class method.
