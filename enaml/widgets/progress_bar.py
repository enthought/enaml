...
class ProgressBar(Control):
    ...
    """
    #: The minimum value that the progress bar can take.
    #: Extra checks take place to make sure that the user does not
    # :programmatically set :attr:`minimum` > :attr:`maximum`.
    minimum = Property(Int, depends_on='_minimum')

    #: The internal min value storage
    _minimum = Int(0)

    #: The maximum value that the progress bar can take.
    #: Extra checks take place to make sure that the user does not
    #: programmatically set :attr:`minimum` > :attr:`maximum`.
    ...
    def _set_minimum(self, value):
        ...
        if value > self.maximum:
            msg = ("The minimum value of ProgressBar should be smaller than the "
                   "current maximum value({0}), but a value of {1} was given")
    ...
    def _set_maximum(self, value):
        ...
        if value < self.minimum:
            msg = ("The maximum value of ProgressBar should be larger than the "
                   "current minimum value({0}), but a value of {1} was given")
    ...
    def _get_percentage(self):
        ...
        """
        dy = self.maximum - self.minimum
        if dy == 0:
            return 0
        elif self.value == self.maximum:
            return 100
        dx = float(self.value - self.minimum)
        percentage = int(round(100.0 * dx / dy))
        # We already excluded the case where the value was exactly the maximum,
        # so we can't really be at 100%, so round this down to 99% if necessary.
