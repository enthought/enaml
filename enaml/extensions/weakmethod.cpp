...
// A python callable which wraps the weak_methods_dead_ref C function.
// This is initialized in the module init function. This is not declared 
// as a PyObjectPtr since a destructor being run on dlclose() has the 
// potential to call the tp_dealloc slot after interpreter shut down.
static PyObject* dead_ref;


// A callback fired by the object underlyling a weakref used as a key 
// in the weak_methods dict is collected. This removes the entry from 
// the dict and allows the WeakMethod instances to be collected.
static PyObject*
weak_methods_dead_ref( PyObject* self, PyObject* wr_item )
{   
...
// A method structure used for initializing the PyCFunction object which
// wraps the weak_methods_dead_ref callback.
static PyMethodDef
dead_ref_method = {
    "weak_methods_dead_ref", ( PyCFunction )weak_methods_dead_ref, 
    METH_O, "Release internal strong references to WeakMethod instances"
};
...
{   
    ...
    if( ( kwargsptr ) && ( kwargsptr.size() > 0 ) )
    {
        ...
        ostr << static_cast<int>( kwargsptr.size() ) << " given)";
    ...
    if( argsptr.size() != 1 )
    {
        ...
        ostr << static_cast<int>( argsptr.size() ) << " given)";
        return py_type_fail( ostr.str().c_str() );
    }
    ...
    */
    ...
    if( !items )
    {
        ...
        if( !items )
            return 0;
        PyWeakrefPtr selfrefcb( PyWeakref_NewRef( self.get(), dead_ref ) );
        if( !selfrefcb )
            return 0;
...
"WeakMethod(method)\n\n"
"A `WeakMethod` is an object which weakly wraps a method with a\n"
"lifetime bound to the lifetime of the underlying object.\n\n"
"Instances of `WeakMethod` are also weakref-able with a lifetime\n"
"which is also bound to lifetime of the method owner.\n\n"
"If multiple `WeakMethod` instances are requested for the same\n"
"equivalent method object, the same `WeakMethod` instance will be\n"
"returned. This behavior follows normal weakref semantics.\n\n"
"Parameters\n"
"----------\n"
...
initweakmethod( void )
{
    ...
    dead_ref = PyCFunction_New( &dead_ref_method, mod );
    if( !dead_ref )
        return;
    ...
    PyModule_AddObject( mod, "WeakMethod", ( PyObject* )&WeakMethod_Type );
}
